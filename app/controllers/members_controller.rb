class MembersController < ApplicationController
  ...
  def index
    ...
    render json: @members
    # , each_serializer: SimpleMemberSerializer
  end
  ...
  def update
    if @current_user.members.find_by(group_id: @member.group_id,
                                       role: %w[organizer co-organizer]) && @member.update(admin_params)
  ...
  def destroy
    if @current_user.members.find_by(group_id: @member.group_id, role: %w[organizer co-organizer]) && @member.destroy
      render json: @member
    else
